...
// import { useRouter } from "next/navigation"; // Importation inutilisée retirée

// CORRECTION: Modification de la syntaxe d'exportation pour être plus robuste
export default function Page() {
  ...
  const buildQuery = () => {
    ...
    query.set("filters[date][$gte]", start.toISOString());
    query.set("filters[date][$lte]", end.toISOString());
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/sale-transactions?${buildQuery()}`);
      if (!response.ok) {
        throw new Error("Failed to fetch monthly sales");
      }
      const responseData = await response.json();
      setSales(responseData.data);
      setMeta(responseData.meta.pagination);
    } catch (error) { 
      console.log("Failed to fetch monthly sales:", error);
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await axiosInstance.delete(`/api/sale-transactions/${item.id}`);
      await fetchData();
    ...
    <div className="py-4 md:py-6 px-4 lg:px-6">
      <Card className="@container/card">
        <CardHeader>
          ...
          <CardDescription>
            <span>List of this month's sales</span>
          </CardDescription>
        </CardHeader>
